...
router.post('/', (req, res) => {
    db
        ...
        .catch(err => {
            res.status(500).json({ error: 'Your project could not added at this time.' })
        })
});
...
router.delete('/:id', mw.validateProjectID, (req, res) => {
    db
        ...
        .then(del => {
            if(del) {
                res.status(200).json({ message: 'You have successfully deleted your project.' })
            } else {
                res.status(404).json({ message: 'Please use an existing ID.' })
            }
        })
        .catch(err => {
            res.status(500).json({ error: 'This project could not be deleted at this time.' })
        })
});
